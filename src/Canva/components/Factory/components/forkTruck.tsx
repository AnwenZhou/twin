...
const ForkTruck = forwardRef<ForkTruckRef, IFourWayCar>((props, ref) => {
  const { hasGoods, position, liftArmHeight = 0, forkArmHeight = 0, radian = 0, label } = props;
  ...
  const model1 = useMemo(() => {
    ...
    applyMetalLookToObject(model, { metalness: 1, roughness: 0.16, envMapIntensity: 2.5 });
    return model;
  }, [forkFbx]);
  ...
  const model3 = useMemo(() => {
    ...
    model.scale.set(0.05, 0.05, 0.05);
    applyMetalLookToObject(model, { metalness: 1, roughness: 0.22, envMapIntensity: 2.4 });
    return model;
  }, [bodyFbx]);
  return (
    <group ref={rootRef} position={position} rotation-y={radian}>
      {label && <VehicleLabel text={label} offsetY={140} />}
      {/* 叉臂 */}
